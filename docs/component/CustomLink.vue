...
// from https://github.com/chodocs/chodocs/blob/main/docs/.vitepress/theme/components/CustomLink.vue
...
const hrefSource = computed(() => {
  ...
  else if (/bun/.test(href)) return "bun";
  else return "web";
});
...
<template>
  ...
  >
    <div
      class="custom-wrap rounded-lg p-px dark:shadow-lg shadow-black/20 my-4 border-zinc-300 border-solid border-px"
    >
      <div class="rounded-lg dark:bg-black">
        <section
          class="flex group flex-col rounded-md p-4 gap-3 transition duration-500 cursor-pointer"
        >
          <span
          ...
          <div class="flex item-center justify-between">
            <div class="flex items-center gap-1 w-full max-w-[75%]">
              <tabler:brand-bilibili
                v-if="hrefSource === 'bilibili'"
                class="text-blue-600 w-8 h-8"
              />
              <uiw:weixin
                v-if="hrefSource === 'weread'"
                class="text-blue-600 w-8 h-8"
              />
              <fe:youtube
                v-if="hrefSource === 'youtube'"
                class="text-red-600 w-8 h-8"
              />
              <bi:github
                v-if="hrefSource === 'github'"
                class="text-slate-600 w-7 h-7"
              />
              <ph:planet-fill
                v-if="hrefSource === 'zsxq'"
                class="text-emerald-600 w-8 h-8"
              />
              <uiw:weixin
                v-if="hrefSource === 'mpwx'"
                class="text-emerald-600 w-8 h-8"
              />
              <skill-icons:vercel-dark
                v-if="hrefSource === 'vercel'"
                class="text-zinc-800 w-7 h-7"
              />
              <teenyicons:nextjs-solid
                v-if="hrefSource === 'nextjs'"
                class="text-zinc-800 w-7 h-7"
              />
              <tabler:brand-juejin
                v-if="hrefSource === 'juejin'"
                class="text-blue-600 w-7 h-7"
              />
              <skill-icons:typescript
                v-if="hrefSource === 'typescript'"
                class="w-7 h-7"
              />
              <logos:react-query-icon
                v-if="hrefSource === 'react-query'"
                class="w-7 h-7"
              />
              <skill-icons:react-dark v-if="hrefSource === 'react'" class="w-7 h-7" />
              <skill-icons:stackoverflow-dark
                v-if="hrefSource === 'stackoverflow'"
                class="w-7 h-7"
              />
              <vscode-icons:file-type-bun v-if="hrefSource === 'bun'" class="w-7 h-7" />
              <icon-park:add-web v-if="hrefSource === 'web'" class="w-7 h-7" />
              <span
                class="text-ellipsis w-full whitespace-nowrap overflow-hidden text-sm opacity-75 font-500"
                >{{ href }}</span
              >
            </div>
